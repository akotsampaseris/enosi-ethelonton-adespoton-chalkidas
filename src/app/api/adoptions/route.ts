...
export async function POST(request: NextRequest) {
    try {
        ...
        const { fullName, email, phone, address, city, housingType, hasYard, hadPetsBefore, currentPets, workSchedule, whoWillCare, whyAdopt, animalId, animalName, submittedAt } =
            body;

        const organizationEmail = process.env.ORGANIZATION_EMAIL ?? "admin@eeach.gr";
        const fromEmail = process.env.NODE_ENV == "development" ? "onboarding@resend.dev" : "adoptions@eeach.gr";

        // Send email to organization
        const { data: orgEmail, error: orgError } = await resend.emails.send({
            // from: "Αιτήσεις Υιοθεσίας <adoptions@enosi-ethelonton.gr>",
            from: `Αιτήσεις Υιοθεσίας <${fromEmail}>`,
      ...
      `,
        ...
        return NextResponse.json({ error: "Failed to submit adoption application" }, { status: 500 });
    }
}
