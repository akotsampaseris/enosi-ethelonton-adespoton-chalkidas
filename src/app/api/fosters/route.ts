...
export async function POST(request: NextRequest) {
    try {
        ...
        const organizationEmail = process.env.ORGANIZATION_EMAIL ?? "admin@eeach.gr";
        const fromEmail = process.env.NODE_ENV == "development" ? "onboarding@resend.dev" : "fostering@eeach.gr";

        // Send email to organization
        await resend.emails.send({
            from: `Αιτήσεις Φιλοξενίας <${fromEmail}>`,
        ...
        <ul>
          ...
          <li><strong>Ιατρικές ανάγκες:</strong> ${
              canHandleMedical === "yes" ? "Ναι, άνετα" : canHandleMedical === "depends" ? "Εξαρτάται από την περίπτωση" : "Όχι, προτιμώ υγιή ζώα"
          }</li>
          <li><strong>Μεταφορά:</strong> ${canTransport === "yes" ? "Ναι, όποτε χρειαστεί" : canTransport === "sometimes" ? "Μερικές φορές" : "Όχι, χρειάζομαι βοήθεια"}</li>
        </ul>
      ...
      `,
        ...
        return NextResponse.json({ error: "Failed to submit foster application" }, { status: 500 });
    }
}
