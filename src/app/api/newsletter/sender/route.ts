...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!senderApiToken || !senderGroupId) {
            ...
            return NextResponse.json({ error: "Η υπηρεσία newsletter δεν είναι διαθέσιμη αυτή τη στιγμή" }, { status: 500 });
        }

        // Subscribe to Sender.net
        const response = await fetch(`${senderApiUrl}/subscribers`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            if (response.status === 422 && data.message?.includes("already exists")) {
                return NextResponse.json({ error: "Αυτή η διεύθυνση email είναι ήδη εγγεγραμμένη" }, { status: 400 });
            }

            console.error("Sender.net API error:", data);
            return NextResponse.json({ error: "Αποτυχία εγγραφής. Παρακαλώ δοκιμάστε ξανά." }, { status: response.status });
        }
