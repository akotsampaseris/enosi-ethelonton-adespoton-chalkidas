            ...
            {Object.entries(faqsByCategory).map(([category, faqs]) => (
                <div key={category}>
                    ...
                    <div className="space-y-4">
                        {faqs.length === 0 ? (
                            <div className="text-center py-8 text-gray-500 bg-gray-50 rounded-2xl">Δεν υπάρχουν ερωτήσεις σε αυτή την κατηγορία ακόμα.</div>
                        ) : (
                            faqs.map((faq) => (
                                <div key={faq._id} className="bg-white rounded-2xl border border-gray-200 hover:border-pink-300 transition-colors overflow-hidden">
                                    <button onClick={() => toggleFAQ(faq._id)} className="w-full flex items-start justify-between gap-4 p-6 text-left">
                                        <h3 className="flex-1 text-lg font-bold text-gray-900">{faq.question}</h3>
                                        <ChevronDown className={`flex-shrink-0 text-pink-500 transition-transform ${openFAQs.has(faq._id) ? "rotate-180" : ""}`} size={24} />
                                    </button>
